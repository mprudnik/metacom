...
class Metacom extends EventEmitter {
  ...
  #callbackHandler = ({ id, meta, result }) => {
    const promised = this.calls.get(id);
    if (!promised) return;
    ...
    this.calls.delete(id);
    clearTimeout(timeout);
    resolve({ ...result, meta }); // should meta be passed here?
  };

  #eventHandler = ({ unit, eventName, meta, data }) => {
    const metacomUnit = this.api[unit];
    if (metacomUnit) metacomUnit.emit(eventName, { ...data, meta }); // should meta be passed here?
  };

  #streamHandler = async ({ id, name, size, status }) => {
    const stream = this.streams.get(id);
