...
class MetaWritable extends EventEmitter {
  ...
  init() {
    ...
    const packet = protocol.serialize('stream', { id, name, size });
  ...
  write(data) {
    ...
    const packet = protocol.serialize('stream', { id: this.id, status: 'end' });
    this.transport.send(JSON.stringify(packet));
  }

  terminate() {
    const raw = { id: this.id, status: 'terminate' };
    const packet = protocol.serialize('stream', raw);
    this.transport.send(JSON.stringify(packet));
  }
}
