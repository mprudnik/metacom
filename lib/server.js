...
class Client extends EventEmitter {
  ...
  createStream(name, size) {
    ...
    const stream = new MetaWritable(id, name, size, this.#transport.connection);
...
class Server {
  ...
  message(client, raw) {
    if (Buffer.compare(EMPTY_PACKET, raw) === 0) {
      client.send('{}');
      return;
    }
    ...
    if (!handler) {
      const error = new Error('Packet structure error');
      client.error(400, { ...data, error });
      return;
    }
    handler(client, data);
  }

  async rpc(client, { id, unit, version = '*', name, args }) {
    const proc = this.application.getMethod(unit, version, name);
    if (!proc) {
      client.error(404, { id });
      return;
    }
    ...
    const packet = protocol.serialize('callback', { id, result });
    client.send(packet);
    this.console.log(`${client.ip}\t${name}`);
  }

  async stream(client, { id, name, size, status }) {
    const tag = id + '/' + name;
    try {
  ...
  request(client, transport, data) {
    ...
    const packet = protocol.serialize('call', { id: 0, unit, name, args });
