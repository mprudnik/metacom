...
class Server {
  ...
  message(client, msg) {
    ...
    // format validation can be done in deserialize
  ...
  async rpc(client, { callId, iface, ver = '*', method, data }) {
    const proc = this.application.getMethod(iface, ver, method);
    if (!proc) {
      client.error(404, { id: callId });
      return;
    }
    const context = client.createContext();
    if (!client.session && proc.access !== 'public') {
      client.error(403, { id: callId });
      return;
    }
    try {
      await proc.enter();
    } catch {
      client.error(503, { id: callId });
      return;
    }
    let result = null;
    try {
      result = await proc.invoke(context, data);
    } catch (error) {
      if (error.message === 'Timeout reached') {
        error.code = error.httpCode = 408;
      }
      client.error(error.code, { id: callId, error });
      return;
    } finally {
      proc.leave();
    }
    if (result?.constructor?.name === 'Error') {
      ...
      client.error(code, { id: callId, error: result, httpCode });
      return;
    }
    const packet = protocol.serialize('callback', { callId, data: result });
    client.send(packet);
    this.console.log(`${client.ip}\t${method}`);
  }

  async stream(client, { streamId, name, size, status }) {
    const tag = streamId + '/' + name;
    try {
      ...
      client.error(400, { id: streamId, error, pass: true });
    }
  }
  ...
  request(client, transport, data) {
    ...
    const packet = { id: 0, method: unit + '/' + method, args }; // what type?
